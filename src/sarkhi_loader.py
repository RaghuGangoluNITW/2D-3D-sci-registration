"""
sarkhi_loader.py — Patient Loader for SARKHI Dataset
=====================================================
Loads SARKHI's pre-op CT (NRRD) + intra-op C-arm NRRD images and wraps
them into the DeepFluoroSpecimen / DeepFluoroProjection interface.

Data layout (from ProcessedData zip):
  CT (pre-op NRRD):
    SARKHI/CT label/4 L_Spine  1.0  I26s  3.nrrd
      512×512×430, spacing 0.371×0.371×0.600 mm

  3D landmarks (vertebral centroids, Slicer mrk.json, LPS):
    SARKHI/CT label/CENTROID.mrk.json
      Labels: L5, L4, L3, L2, L1, D12

  Intra-op X-rays (NRRD, 1024×1024) + 2D landmark annotations (mrk.json):
    SARKHI/XRAY Label/XRAY-AP/AP-SET-1/  → 0014.nrrd  (L5,L4,L3,L2)
    SARKHI/XRAY Label/XRAY-AP/AP-SET-2/  → 0063.nrrd  (L5,L4,L3,L2,L1) ← FULL
    SARKHI/XRAY Label/XRAY-AP/AP-SET-3/  → 0064.nrrd  (L5,L4,L3,L2,L1) ← FULL
    SARKHI/XRAY Label/XRAY-LAT/LAT-SET-1/ → 0046.nrrd (L5,L4,L3,L2)
    SARKHI/XRAY Label/XRAY-LAT/LAT-SET-2/ → 0034.nrrd (L5,L4,L3,L2)
    SARKHI/XRAY Label/XRAY-LAT/LAT-SET-3/ → 0006.nrrd (L5,L4,L3)

  2D landmark format: mrk.json with LPS coordinateSystem but position=(u, v, 0)
  where u,v are pixel coordinates in the 1024×1024 image.

Camera model (assumed Ziehm Vision FD — same as Arjun, no DICOM metadata):
  SID     = 1110 mm
  Pixel spacing = 0.2 mm/px  at 1024 px reference
  Fx = Fy = 1110 / 0.2 = 5550 px
  Cx = Cy = (1024-1)/2 = 511.5

EPnP initialization:
  For sets with ≥4 annotated landmarks: cv2.solvePnP (EPNP).
  Fallback: anatomy centroid pose (AP=0°, LAT=90° azimuth).

PDE evaluation:
  Since 2D annotations are available, PDE (mm) = reprojection error × pixel_spacing_mm.
"""
...
# ---------------------------------------------------------------------------
# Camera constants (Ziehm Vision FD assumed)
# ---------------------------------------------------------------------------

SARKHI_SID_MM:   float = 1110.0
SARKHI_PIX_MM:   float = 0.2            # pixel spacing at 1024 reference
SARKHI_IMG_SIZE: int   = 1024

SARKHI_FX: float = SARKHI_SID_MM / SARKHI_PIX_MM   # 5550 px
SARKHI_FY: float = SARKHI_SID_MM / SARKHI_PIX_MM
SARKHI_CX: float = (SARKHI_IMG_SIZE - 1) / 2.0      # 511.5
SARKHI_CY: float = (SARKHI_IMG_SIZE - 1) / 2.0

SARKHI_K: np.ndarray = np.array([
    [SARKHI_FX, 0.,        SARKHI_CX],
    [0.,        SARKHI_FY, SARKHI_CY],
    [0.,        0.,        1.        ],
], dtype=np.float64)
...
_PROC_BASE = Path('/home/supermicro/Documents/2D_3D_Raghu/data/ProcessedData/Processed Data 2D-3D/SARKHI')

CT_NRRD    = _PROC_BASE / 'CT label' / '4 L_Spine  1.0  I26s  3.nrrd'
LM_3D_JSON = _PROC_BASE / 'CT label' / 'CENTROID.mrk.json'
...
class SarkhiProjection(DeepFluoroProjection):
    """SARKHI projection using fixed 1024×1024 Ziehm intrinsics."""

    def project(self, pts3d_world: np.ndarray) -> np.ndarray:
        pts = np.atleast_2d(pts3d_world)
        P_cam = (self.R_proj @ xzy(pts).T).T + self.t_proj
        u = SARKHI_FX * P_cam[:, 0] / P_cam[:, 2] + SARKHI_CX
        v = SARKHI_FY * P_cam[:, 1] / P_cam[:, 2] + SARKHI_CY
...
def project_world_sarkhi(pts3d_world: np.ndarray,
                          R: np.ndarray,
...
def compute_pde_sarkhi(proj: SarkhiProjection,
                        R_cand: np.ndarray,
                        t_cand: np.ndarray,
                        pts3d: np.ndarray,
                        lm_names: List[str]) -> Dict[str, float]:
    """PDE (mm) between candidate pose and GT 2D annotations."""
    uv_pred = project_world_sarkhi(pts3d, R_cand, t_cand)
    pde: Dict[str, float] = {}
    for i, name in enumerate(lm_names):
        if name in proj.gt_landmarks_2d:
            gt = proj.gt_landmarks_2d[name]
            if 0 <= gt[0] < SARKHI_IMG_SIZE and 0 <= gt[1] < SARKHI_IMG_SIZE:
                err_px = float(np.linalg.norm(uv_pred[i] - gt))
                pde[name] = err_px * SARKHI_PIX_MM
    return pde


def mean_pde_sarkhi(proj, R, t, pts3d, lm_names) -> float:
    d = compute_pde_sarkhi(proj, R, t, pts3d, lm_names)
...
def _load_mrk_json_2d(json_path: Path) -> Dict[str, np.ndarray]:
    ...
    The mrk.json stores position=(u, v, 0) in 'LPS' coordinateSystem,
    but u,v are pixel coordinates in the 1024×1024 image.
    """
    with open(json_path) as f:
        d = json.load(f)
    cps = d['markups'][0]['controlPoints']
...
def _load_nrrd_xray(nrrd_path: Path) -> np.ndarray:
    """Load 1024×1024 NRRD X-ray → float32 [0,1]."""
    img = sitk.ReadImage(str(nrrd_path))
    arr = sitk.GetArrayFromImage(img).squeeze()   # (1024, 1024)
...
def _solve_pnp_sarkhi(pts3d_world: np.ndarray,
                       pts2d: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """EPnP using SARKHI camera intrinsics. pts3d_world in LPS mm."""
...
def _reproj_error_sarkhi(pts3d: np.ndarray, pts2d: np.ndarray,
                          R: np.ndarray, t: np.ndarray) -> float:
    uv_pred = project_world_sarkhi(pts3d, R, t)
...
def _anatomy_pose(lm_3d: Dict[str, np.ndarray],
    ...
    """Anatomy-centred (R, t) pose as EPnP fallback."""
    ...
    src_world = centroid + SARKHI_SID_MM * src_dir
...
class SarkhiLoader:
    """
    Load SARKHI pre-op CT (NRRD) + intra-op NRRD X-rays with 2D mrk.json labels.

    Usage::
        loader = SarkhiLoader()
        spec   = loader.load()                           # all available sets
        spec   = loader.load(sets=['AP-SET-2', 'AP-SET-3'])  # specific sets
    """

    # All possible sets in order
    AP_SETS  = ['AP-SET-1',  'AP-SET-2',  'AP-SET-3']
    LAT_SETS = ['LAT-SET-1', 'LAT-SET-2', 'LAT-SET-3']
    ...
    def _discover_sets(self) -> List[Tuple[str, Path, Path, str]]:
        ...
        Returns only sets where both image and centroid JSON exist.
        """
        found = []
    ...
    def load(self,
        ...
        """
        Load SARKHI dataset as a DeepFluoroSpecimen.

        Args:
            sets    : list of set names to load, e.g. ['AP-SET-2', 'LAT-SET-1'].
                      None = all sets where image + labels exist.
            verbose : print progress.

        Returns:
            DeepFluoroSpecimen with SarkhiProjection objects.
        """
        if verbose:
            print("[SARKHI] Loading specimen ...")
        ...
        projections: List[SarkhiProjection] = []
        for idx, (set_name, nrrd_path, json_path, view_tag) in enumerate(all_sets):
            ...
            # Load 2D landmarks (pixel coords)
            ...
            if len(common_labels) >= 4:
                ...
                try:
                    R_init, t_init = _solve_pnp_sarkhi(pts3d, pts2d)
                    reproj_err = _reproj_error_sarkhi(pts3d, pts2d, R_init, t_init)
                    if verbose:
                        pde_mm = reproj_err * SARKHI_PIX_MM
